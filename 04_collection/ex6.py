...
# result={result[i]:i for i in result}
result={v:k for k,v in result.items()}
print(result)
# 점수 90 이상만 필터링하기
scores={"국어":90,"영어":95,"수학":89}
...
result={k:"합격" for k in scores if scores[k]>=60}
...
# 2) dict 컴프리헨션
stock.update({item:stock.get(item,0)+qyt for item ,qyt in incoming.items()})
